...
if(!empty($_GET)) {
        debug_to_console("GET - request");
        $for_data = $_GET;
    } elseif (!empty($_POST)) {
        debug_to_console("Post - request");
        $for_data = $_POST;
    } 
debug_to_console($for_data);

$token = $for_data['token'];
$secret = $for_data['secret'];
...
switch ($for_data['form_name']) {
    case 'FIO':
      // Стандартизовать ФИО
      $result = $dadata->clean("name", $for_data['user_name']." ".$for_data['user_second_name']." ".$for_data['user_last_name']); 
      break;
    case 'INN':
      // Найти компанию по ИНН
      $fields = array("query" => $for_data['inn'], "count" => 5);
